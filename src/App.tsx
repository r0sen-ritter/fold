...
const App = () => {
  ...
  const childFolders = currentFolder
    ? folders.filter((folder) =>
        currentFolder.childFolderId.includes(folder.id)
      )
    : [];
    ...
    if (newFolderName.trim() !== "") {
      ...
      const updatedFolders = folders.map((folder) =>
        folder.id === currentFolderId
          ? {
              ...folder,
              childFolderId: [...folder.childFolderId, newFolder.id],
            }
          : folder
      );
  ...
  const getFolderPath = (folderId: string): Folder[] => {
    ...
    while (currentFolder) {
      ...
      currentFolder = folders.find(
        (folder) => folder.id === (currentFolder as Folder).parentFolderId
      );
    }
  ...
  return (
    <div className="grid grid-cols-1 mt-10 max-w-4xl mx-auto">
      <div className="flex justify-items-center p-2 w-2/4 m-auto shadow-sm justify-center content-center mt-5 border-2 rounded">
        <button
          className="m-auto shadow-md hover:text-white hover:scale-110 transition duration-150 hover:bg-blue-400 rounded p-2"
      ...
      <div
        className="p-1 min-h-10 min-w-20 mt-5 grid grid-cols-3"
        style={{ gap: "1rem" }}
      >
        {childFolders.map((folder) => (
          <div
            className="p-2 justify-items-center relative m-2 border-1 shadow-md hover:scale-105 transition duration-300 justify-between flex rounded"
            key={folder.id}
            onClick={() => handleFolderClick(folder.id)}
            style={{ backgroundColor: folder.folderColorHex }}
          >
            ...
            <svg
              onClick={(e) => handleIconClick(e, folder.id)}
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
              stroke-width="1.5"
            ...
            {openDropdownId === folder.id && (
              <div
                className="dropdown-menu absolute right-[-2rem] top-6"
                style={dropdownStyles}
              >
                {Object.keys(colorHexList).map((key) => (
                  <div
                    key={key}
                    className="dropdown-item hover:shadow-md hover:bg-slate-400 rounded"
                    onClick={(event) => {
                      event.stopPropagation();
                ...
                <div
                  className=" p-2 rounded hover:scale-105 transition duration-150 hover:bg-red-600 hover:text-white"
                  onClick={(e) => {
                    e.stopPropagation();
                    openModal(folder);
                  }}
